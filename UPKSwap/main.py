...
def cmd_revert(args, items):
    ...
    if target is None:
        return 1
    return _do_revert(target)


def _do_revert(target) -> int:
    bak = COOKED / f"{target.asset_package}.bak"
    dst = COOKED / target.asset_package
    if bak.exists():
